    ...
    def __init__(self):
        ...
        if "homography_pixel_to_robot_mm" in data:
            ...
            self.get_logger().warn(
                'No homography in calibration file — falling back to 3D ray intersection.'
            )
        ...
        try:
            ...
            self.get_logger().warn('cv2.aruco unavailable — ArUco overlay disabled')
    ...
    def timer_callback(self):
        ...
        for cls_name, instances in per_class_raw.items():
            ...
            for idx, raw in enumerate(instances):
                ...
                # Camera → robot XY: use homography if available, else ray intersection
