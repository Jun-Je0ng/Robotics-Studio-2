...
class StatusGui(Node):
    ...
    def _get_eef_pose(self):
        """Return (position, rotvec) by TF2 lookup of gripper_tcp in base frame."""
        ...
        if s < 1e-6:
            ...
            rotvec = [qx/s * angle, qy/s * angle, qz/s * angle]
        return position, rotvec

    def _save_bin(self, type_name: str):
        try:
            position, rotvec = self._get_eef_pose()
        ...
        data[type_name] = {"position": position, "orientation": rotvec}

        try:
            with open(BIN_POSES_PATH, 'w') as f:
                json.dump(data, f, indent=4)
        except Exception as e:
            self._log(f"Failed to write bin_poses.json: {e}", "err")
            return
